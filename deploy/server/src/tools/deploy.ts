...
const PROJECT_ROOTS = {
  commander: join(REPO_ROOT, "commander-collector"),
  portfolio: join(REPO_ROOT, "website/rickwphillips.com"),
  grandkid: join(REPO_ROOT, "grandkid-arcade"),
} as const;
...
const checkGitClean = (cwd: string): Promise<{ clean: boolean; output: string }> => {
...
const checkCommanderMigration = (): { ok: boolean; message: string; version?: string } => {
  ...
  try {
    ...
    return {
      ok: false,
      message: `Failed to parse ${pkgPath}: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
    ...
    async ({ target, flags, skip_preflight }) => {
          ...
          content: [
            {
              type: "text",
              text: JSON.stringify(
                { status: "ERROR", message: `Unknown target: ${target}` },
                null,
                2,
              ),
            },
      ...
      if (!preflightSkipped) {
        ...
        for (const p of projects) {
          gitChecks[p] = await checkGitClean(PROJECT_ROOTS[p]);
        }
        ...
        const migrationFailed =
          projects.includes("commander") &&
          (preflight.migration as { ok: boolean }).ok === false;
