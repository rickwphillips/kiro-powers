...
const URI = "commander://project-context";

const BODY = `# Commander Collector — Project Context

## Project Location
Repo: \`commander-collector/\` (turborepo monorepo)
...
## Migrations
- Files live at \`commander-collector/migrations/v<version>.sql\` at REPO ROOT
- NOT in \`apps/core/migrations/\` — deploy script reads root only; wrong location = silently skipped
- Filename MUST match \`apps/core/package.json\` version exactly
...
export const registerProjectContextResource = (server: McpServer) => {
  server.registerResource(
    ...
    URI,
    {
      ...
      description:
        "Architecture, conventions, deploy, type system, and components for the Commander Collector MTG game-tracking app.",
